...
class RedisQueueConsumer:
    def __init__(self):
        ...
        # Queue names
        self.detection_queue = "det#vaccine_monitor_test"
    ...
    def log_to_file(self, message_data, file, prin = False):
        os.makedirs(f"logs/test/{file}", exist_ok=True)
        try:
            if prin:
            ...
            with open(f'logs/test/{file}/{dt_string}.log', 'a+', encoding='utf-8') as f:
                f.write(json.dumps(message_data, ensure_ascii=False) + '\n')
        
        except Exception as e:
            print(f"--> INFO: Log to error_message ...")
            with open(f'logs/test/error_message.log', 'a+', encoding='utf-8') as f:
