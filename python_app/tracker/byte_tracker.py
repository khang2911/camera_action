...
from python_app.tracker.utils.matching import xywh2ltwh
from python_app.tracker.basetrack import BaseTrack, TrackState
from python_app.tracker.utils import matching
from python_app.tracker.utils.kalman_filter import KalmanFilterXYAH
...
class STrack(BaseTrack):
    ...
    def xywha(self) -> np.ndarray:
        ...
        if self.angle is None:
            print("`angle` attr not found, returning `xywh` instead.")
            return self.xywh
...
class BYTETracker:
    ...
    def __init__(self, config_path, frame_rate: int = 30):
        ...
        Examples:
            ...
            >>> tracker = BYTETracker(args, frame_rate=30)
        """
        # self.config = ByteTrackConfig(config_path)
    ...
    def update(self, results, img: np.ndarray | None = None, feats: np.ndarray | None = None) -> np.ndarray:
        ...
        # scores = results.conf
        scores = np.array([x.conf for x in results])
        tlbrs = np.array([x.bbox for x in results])
        # class_inds = np.array([x.class_id for x in results])
        ...
        outputs = np.asarray([x.result for x in self.tracked_stracks if x.is_activated], dtype=np.float32)
        # print(outputs)
        # if not len(outputs):
        #     return []
        # tracking_output = outputs[outputs[:, -1].argsort()]
        return outputs
    ...
    def init_track(self, results, img: np.ndarray | None = None) -> list[STrack]:
        ...
        scores = [x.conf for x in results]
        tlbrs = [x.bbox for x in results]
        class_inds = [x.class_id for x in results]
        
        # bboxes = results.xywhr if hasattr(results, "xywhr") else results.xywh
        bboxes = tlbrs
        
        bboxes = np.concatenate([bboxes, np.arange(len(bboxes)).reshape(-1, 1)], axis=-1)
        return [STrack(xywh, s, c) for (xywh, s, c) in zip(bboxes, scores, class_inds)]
...
if __name__ == '__main__':
    ...
    class DetResult:
        def __init__(self, bbox, conf, class_id):
            ...
            self.conf = float(conf)                   # confidence
            self.class_id = int(class_id)             # class index

    tracker = BYTETracker(config_path='./bytetrack.yaml', frame_rate=13)

    results = [
        DetResult([100, 150, 200, 300], 0.92, 0),  # object 1
        DetResult([400, 120, 480, 280], 0.65, 0),  # object 2
        DetResult([50,  80,  120, 180], 0.45, 0),  # object 3
        ]
    
    for i in range(3):
        # results = [
        #     DetResult([100+i*5, 150, 200+i*5, 300], 0.92, 0),
        #     DetResult([400+i*3, 120, 480+i*3, 280], 0.65, 0),
        #     DetResult([50+i*2, 80, 120+i*2, 180], 0.45, 0),
        # ]
